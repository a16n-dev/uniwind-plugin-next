...
vi.mock('../uniwind/src/utils/stringifyThemes', () => ({
  stringifyThemes: vi.fn((themes) => `[${themes.map((t: string) => `'${t}'`).join(', ')}]`),
}));
...
describe('UniwindWebpackPlugin', () => {
  ...
  describe('apply', () => {
    ...
    it('should generate CSS and DTS files on beforeCompile', async () => {
      ...
      const { buildDtsFile } = await import('../uniwind/src/utils/buildDtsFile');
      const { stringifyThemes } = await import('../uniwind/src/utils/stringifyThemes');
