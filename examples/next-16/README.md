...
By default, `react-native-web` injects styles on the client, leading to a flash of unstyled content (FOUC) on the initial page load. To avoid this, we can inject the react-native-web styles on the server. See the implementation of `src/app/ReactNativeWebLayeredStyleSheet.tsx` for details.
